...
class DataBlock:
    ...
    def getQueueTime(self):
        ...
        for arrived, sendReady in zip(self.checkPoints, self.checkPointsSend):  # rest of the steps
            ...
            queueLatency[1].append(sendReady - arrived) # record the latency of every step

        self.queueLatency = queueLatency
        return queueLatency

    def getTotalTransmissionTime(self): # here is the total transmission time
        totalTime = 0
...
class Gateway:
    ...
    def __init__(self, name: str, ID: int, latitude: float, longitude: float, totalX: int, totalY: int, totalGTs, env, totalLocations, earth, pathMetric):
        ...
        # comm attributes
        self.dataRate = None
        self.gs2ngeo = RFlink(  # Ground Station to Non-GEO satellite
            frequency=30e9,
            bandwidth=500e6,
            maxPtx=20,
            aDiameterTx=0.33,
...
class Earth:
    def __init__(self, env, img_path, gt_path, constellation, inputParams, deltaT, totalLocations, getRates = False, window=None):
        ...
        if inputParams['Locations'][0] != 'All':  # inputParams['Locations'][0] only use city in input.csv file
            for i, location in enumerate(gateways['Location']):
                for name in inputParams['Locations']:
                    if name in location.split(","):
            ...
            for i in range(len(gateways['Latitude'])):   # every location in the Gateways.csv file
        ...
        self.pathParam = inputParams['Pathing'][0]  # weight model
    ...
    def linkCells2GTs(self, distance):
        ...
        for i, gt in enumerate(self.gateways): # self.gateway used GT Gateway
        ...
        for cells in self.cells:
            for cell in cells:
                if cell.gateway is not None:
                    cell.gateway[0].addCell([(math.degrees(cell.latitude), # (groundstation, distance)
                                                     math.degrees(cell.longitude)),
                                                    cell.users,
                                                    cell.gateway[1]])
                    # print(cell.gateway[0].ID)
    ...
    def linkSats2GTs(self, method):
        ...
        if method == "Greedy":
            ...
            for i, GT in enumerate(self.gateways):
                GT.orderSatsByDist(self.LEO)
                for val, entry in enumerate(GT.satsOrdered): # entry: (d_GSL, sat, [index])
                    SxGT[i][entry[2][0]] = val
                    # i is the index of the GT, entry[2][0] is the index of the satellite in the sats list
            # find assignment of GSL which minimizes the cost from the cost matrix
            rowInd, colInd = linear_sum_assignment(SxGT)
            # rowInd is the indices of the gateways, colInd is the indices of the satellites
            # link satellites and GTs
            for i, GT in enumerate(self.gateways):
                if SxGT[rowInd[i]][colInd[i]] < len(GT.satsOrdered):
                    sat = GT.satsOrdered[SxGT[rowInd[i]][colInd[i]]] # (distance, satellite, [index])
...
def initialize(env, popMapLocation, GTLocation, distance, inputParams, movementTime, totalLocations):
    ...
    earth.linkCells2GTs(distance) # link cells to GTs based on distance
    earth.linkSats2GTs("Optimize") # link satellites to GTs based on distance and coverage
    ...
    for plane in earth.LEO:
        for sat in plane.sats:
            ...
            for sat2 in sats:
                ...
                if sat2.ID in neighbors: # sat2 is a neighbor of sat
                    dataRate = nx.path_weight(graph,[sat2.ID, sat.ID], "dataRateOG")
                    distance = nx.path_weight(graph,[sat2.ID, sat.ID], "slant_range")
...
def markovianMatchingTwo(earth):
    ...
    shannonRate     = get_data_rate(slant_range_los, interISL)                      # max dataRate bps
...
def createGraph(earth):
    ...
    ###############################
    for plane in earth.LEO:
        for sat in plane.sats: # sat is an instance of the satellite class
...
def main(outputPath):
    ...
    blockPath = outputPath + "/Congestion_test/{} {}s/".format(pathing, int(testLength))
